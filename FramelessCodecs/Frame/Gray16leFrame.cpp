...
Gray16leFrame& Gray16leFrame::operator=(Frame& const src) {
...
uint8_t Gray16leFrame::getIntensityAsByte(int plane, int i) const {
	return (plane == 0) ? static_cast<uint8_t>(intensityAt(i) >> 8) : 0x80;
}

uint8_t Gray16leFrame::getIntensityAsByte(int plane, int x, int y) const {
	return (plane == 0) ? static_cast<uint8_t>(intensityAt(x, y) >> 8) : 0x80;
}

uint16_t Gray16leFrame::getIntensityAs16Bits(int plane, int i) const {
	return intensityAt(plane, i);
}

uint16_t Gray16leFrame::getIntensityAs16Bits(int plane, int x, int y) const {
	return intensityAt(plane, x, y);
}

double Gray16leFrame::getIntensityAsDouble(int plane, int i) const {
	return static_cast<double>(intensityAt(plane, i)) / 0xFFFF;
}

double Gray16leFrame::getIntensityAsDouble(int plane, int x, int y) const {
